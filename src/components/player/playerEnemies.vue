      ...
      >
        ...
        >
          ...
          <template v-slot:item="{item}">
            <tr v-if="item.status !== 'false'"
                :class="hasPlayAllGames(item.playedGames)">
              ...
              <td>
                <template v-if="Array.isArray(item.playedGames)">
                  <div
                      v-for="(game, i) in item.playedGames"
                      :key="i"
                      class="ma-1 gameBudge font-weight-bold"
                      :class="getGameResultBudge(game.result)"
                  >
                    {{ game.result }}
                  </div>
                </template>
...
export default {
  ...
  computed: {
    teams() {
      const res = {};
      for (let team in this.items) {
        if (this.items[team].title !== this.currentTeam) {
          res[team] = this.items[team];
        }
      }
      return res;
    }
  ...
  methods: {
    ...
    hasPlayAllGames(games) {
      if (Array.isArray(games) && games.length === 1) {
        return 'enemy-line grey darken-1 grey--text--darken-2'
      }
    }
  },
}
